...
{
""";
        for j in kinbody.GetJoints():
            l = j.GetLimits();
            code += "{ .limitMin="+str(l[0][0]/3.14159265358979*180)+", .limitMax="+str(l[1][0]/3.14159265358979*180)+" },\n"
        code += "};\n"
        return code

    def generate(self, solvertree, kinbody):
...
#define IKFAST_NUM_DOF """ + str(4) + """
#define IKFAST_NUM_JOINTS """ + str(len(solvertree.freejointvars)+len(solvertree.solvejointvars)) + """
#define IKFAST_NUM_FREE_VARS """ + str(len(solvertree.freejointvars)) + """
...
""")%(self.version,str(datetime.datetime.now()),self.version)
        if kinbody is not None: 
            code += self.GetJointDefs(kinbody)
...
"""
    ...
    def getClassInit(self,node,iktype,userotation=7,usetranslation=7):
        ...
        code += "IKFAST_API int GetNumJoints() { return IKFAST_NUM_JOINTS; }\n\n"
        code += "IKFAST_API int GetIkRealSize() { return sizeof(IkReal); }\n\n"
        code += 'IKFAST_API int GetIkType() { return 0x%x; }\n\n'%iktype
       
        # For C, we need this to be in function scope
        usedvars = []
        ...
        self.computeIkVars = 'static IkReal ' + ','.join(usedvars) + ';\n'
        self.computeIkVars += 'static unsigned char ' + ','.join('_i%s[2], _n%s'%(var[0].name,var[0].name) for var in node.solvejointvars+node.freejointvars) + ';\n\n'
        return code

    def GetIkFunctionPreamble(self, node):
        code = "int ComputeIk(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, IkSolutionList * solutions) {\n"
        for var in node.solvejointvars:
            code += '%s=NAN; _i%s[0] = -1; _i%s[1] = -1; _n%s = -1; '%(var[0].name,var[0].name,var[0].name,var[0].name)
        for i in range(len(node.freejointvars)):
            name = node.freejointvars[i][0].name
            code += ' _i%s[0] = -1; _i%s[1] = -1; _n%s = 0; '%(name,name,name)
        code += "\nint dummyiter;\nfor(dummyiter = 0; dummyiter < 1; ++dummyiter) {\n"
        code += "    IkSolutionList_Clear(solutions);\n"
        return code
    ...
    def generateChain(self, node):
        ...
        for name,functioncode in self.functions.iteritems():
            # put functions before solver
            code = functioncode + code
        code = self.computeIkVars + code
...
"""
    ...
    def generateSolverIKChainAxisAngle(self, node):
        ...
        if node.anglefk and node.Pfk:
            code += self.getFKFunctionPreamble()
            code += "int dummyiter;\n"
            code += "for(dummyiter = 0; dummyiter < 1; ++dummyiter) {\n"
            allvars = node.solvejointvars + node.freejointvars
            allsubs = [(v[0],Symbol('j[%d]'%v[1])) for v in allvars]
            eqs = []
            for eq in node.Pfk[0:3]:
    ...
    def generateConditionedSolution(self, node):
        ...
        if allnumsolutions > 1:
            if allnumsolutions >= 256:
                log.error('num solutions is %d>=256, which exceeds unsigned char',allnumsolutions)
        #code += 'int i%s;\n'%name
        code += 'for(i%s = 0; i%s < numsolutions%s; ++i%s)\n{\n'%(name,name,name,name)
        code += 'if( !%svalid[i%s] )\n{\n    continue;\n}\n'%(name,name)
        code += '_i%s[0] = i%s; _i%s[1] = -1;\n'%(name,name,name)
        # check for a similar solution
        code += 'int ii%s;\n'%name
        code += 'for(ii%s = i%s+1; ii%s < numsolutions%s; ++ii%s)\n{\n'%(name,name,name,name,name)
        code += 'if( %svalid[ii%s] && IKabs(c%sarray[i%s]-c%sarray[ii%s]) < IKFAST_SOLUTION_THRESH && IKabs(s%sarray[i%s]-s%sarray[ii%s]) < IKFAST_SOLUTION_THRESH )\n{\n    %svalid[ii%s]=false; _i%s[1] = ii%s; break; \n}\n'%(name,name,name,name,name,name,name,name,name,name,name,name,name,name)
        code += '}\n'
        code += '%s = %sarray[i%s]; c%s = c%sarray[i%s]; s%s = s%sarray[i%s];\n\n'%(name,name,name,name,name,name,name,name,name)
        if AddHalfTanValue:
    ...
    def generatePolynomialRoots(self, node):
        ...
        code = 'static IkReal op[%d+1], zeror[%d];\nint numroots;\n'%(D,D)
        numevals = 0
        ...
        code += '_n%s = %d;\n'%(name,allnumsolutions)
        #code += 'int i%s;\n'%name
        code += 'for(i%s = 0; i%s < numsolutions; ++i%s)\n    {\n'%(name,name,name)
        code += 'if( !%svalid[i%s] )\n{\n    continue;\n}\n'%(name,name)
        code += '    %s = %sarray[i%s]; c%s = c%sarray[i%s]; s%s = s%sarray[i%s];\n'%(name,name,name,name,name,name,name,name,name)
        if node.AddHalfTanValue:
            code += 'ht%s = IKtan(%s/2);\n'%(name,name)
        ...
        # check for a similar solution
        code += 'int ii%s;\n'%name
        code += 'for(ii%s = i%s+1; ii%s < numsolutions; ++ii%s)\n{\n'%(name,name,name,name)
        code += 'if( %svalid[ii%s] && IKabs(c%sarray[i%s]-c%sarray[ii%s]) < IKFAST_SOLUTION_THRESH && IKabs(s%sarray[i%s]-s%sarray[ii%s]) < IKFAST_SOLUTION_THRESH )\n{\n    %svalid[ii%s]=false; _i%s[1] = ii%s; break; \n}\n'%(name,name,name,name,name,name,name,name,name,name,name,name,name,name)
        code += '}\n'
...
#             code += ' )\n{  continue;\n}\n'
        code += 'static IkReal IKFAST_ALIGNED16(matrixinvcoeffs[%d]);\n'%(node.A.shape[0]*node.A.shape[1])
    ...
    def generateRotation(self, node):
        if not node.functionid in self.functions:
            code = 'inline void rotationfunction%d(IkSolutionList* solutions) {\n'%(node.functionid)
            code += 'int rotationiter = 0;\n'
            code += 'for(rotationiter = 0; rotationiter < 1; ++rotationiter) {\n'
    ...
    def generateStoreSolution(self, node):
        ...
        if node.checkgreaterzero is not None and len(node.checkgreaterzero) > 0:
            ...
            self.dictequations = origequations
        code += '{\n'
        code += 'IkSingleDOFSolution vinfos[%d];\n'%len(node.alljointvars)
        code += 'IkSingleDOFSolution_Init( vinfos, %d );\n'%len(node.alljointvars)
        ...
        for i,varname in enumerate(self.freevars):
            ...
            code += 'vfree[%d] = %d;\n'%(i,ind[0])
        code += 'IkSolutionList_AddSolution(solutions, vinfos, %d, vfree, %d);\n'%(len(node.alljointvars),len(self.freevars))
        code += '}\n'
        return code
                ...
                fcode = """static inline void %s(IkReal rawcoeffs[%d+1], IkReal rawroots[%d], int* numroots)
{
    ...
    IkReal complex D(Q*Q*Q+R*R,0.0);
    IkReal complex Dsqrt = sqrt(D);
    IkReal complex S, T;
    if( cimag(Dsqrt) != 0 ) {
    ...
    *numroots = 0;
    if( IKabs(cimag(B)) < 40*DBL_EPSILON ) {
        rawroots[(*numroots)++] = -a2_3+creal(B);
    }
    complex<IkReal> Arot = complex<IkReal>(0,SQRT_3)*A;
    if( IKabs(cimag(B-Arot)) < 40*DBL_EPSILON {
        rawroots[(*numroots)++] = -a2_3-0.5*creal(B-Arot);
        rawroots[(*numroots)++] = -a2_3-0.5*creal(B+Arot);
    }
}
"""%name
                ...
                fcode = """static inline void %(name)s(IkReal rawcoeffs[%(deg)d+1], IkReal rawroots[%(deg)d], int *numroots)
{
    ...
    double complex coeffs[%(deg)d];
    const int maxsteps = 110;
    int i = 0;
    for(i = 0; i < %(deg)d; ++i) {
        coeffs[i] = rawcoeffs[i+1]/rawcoeffs[0];
    }
    double complex roots[%(deg)d];
    IkReal err[%(deg)d];
    roots[0] = 1;
    ...
    *numroots = 0;
    ...
    for(i = 0; i < %(deg)d; ++i) {
        if( !visited[i] ) {
            ...
            int j;
            for(j = i+1; j < %(deg)d; ++j) {
                if( cabs(roots[i]-roots[j]) < 8*tolsqrt ) {
                    newroot += roots[j];
                    n += 1;
                    visited[j] = true;
                }
...
{
    const IkReal tol = 128.0*DBL_EPSILON;
    static IkReal IKFAST_ALIGNED16(M[16*16]);
    memset( M, 0, sizeof(M) );
    static IkReal IKFAST_ALIGNED16(A[8*8]);
    static IkReal IKFAST_ALIGNED16(work[16*16*15]);
    int ipiv[8];
    int info, coeffindex;
